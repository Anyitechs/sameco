  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await PartsService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.parts.create.success'));
        routerAsync().push('/parts');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await PartsService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.parts.update.success'));
        routerAsync().push('/parts');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
