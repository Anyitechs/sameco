...
import partsListExporterFields from '@/modules/parts/parts-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!partsListExporterFields || !partsListExporterFields.length) {
          throw new Error('partsListExporterFields is required');
        }
        ...
        const response = await PartsService.list(
          filter,
          getters.orderBy,
          null,
          null,
