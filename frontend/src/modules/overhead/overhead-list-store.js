...
import overheadListExporterFields from '@/modules/overhead/overhead-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!overheadListExporterFields || !overheadListExporterFields.length) {
          throw new Error('overheadListExporterFields is required');
        }
        ...
        const response = await OverheadService.list(
          filter,
          getters.orderBy,
          null,
          null,
